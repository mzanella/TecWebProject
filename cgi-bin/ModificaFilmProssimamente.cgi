...
my $nomePagina = "Modifica film in uscita - Amministrazione - InstaFilm";
my $description = "Pagina per la modifica di un film";
...
$nomeBC="<a href=\"Amministrazione.cgi\">Amministrazione</a> <span class=\"breadcrumb\">-&gt;</span> <a href=\"SelezionaFilmDaModificare.cgi\">Seleziona film da modificare</a> <span class=\"breadcrumb\">-&gt;</span> "."Modifica film in uscita";
...
if (DBFunctions::controlAdmin($session->param("utenteEmail")) == 0){
	...
	if (!defined($session->param("id")) && !defined($input{"film"})) {
		...
		$query='/database/filmsProssimamente/filmProssimamente[@id='.$id."]";
		...
		my $alt = $session->param("alt") || $fa->getAttribute("alt");
		...
		print
		"<h1>Modifica film</h1>
		<form id=\"frmInsFilm\" action=\"perl/controlliModificaFilmProssimamente.cgi\" method=\"post\"
		onsubmit=\"return validaForm();\">
			<fieldset>
				<legend>Modifica dati film:</legend>";
