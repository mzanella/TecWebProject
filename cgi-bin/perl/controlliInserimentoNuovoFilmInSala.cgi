...
foreach $name (@names) {
	$input{$name}=DBFunctions::inputControl(Encode::decode_utf8($query->param("commento")));
}
...
if ($error>0){
	...
	$session->header();
	print $session->header(-location=>"../InserisciNuovoFilmSala.cgi");
} else {
	my $src="";
	if ($input{"locandina"} ne ""){
	...
	DBFunctions::aggiungiFilm($src, $input{'locDescr'}, $input{'titolo'}, $dataUscita,
		$input{'paeseDiProduzione'}, $input{'durata'}, $input{'trama'}, $input{'incasso'}, $input{'ValutazioneFilm'},
		$input{'recensione'}, $input{'genere'}, $input{'annoProduzione'}, "si", $input{'attori'}, $input{'regia'});
	...
	my $cookie1 = CGI::Cookie->new(-name=>'CGISESSID',-value=>$session->id);
	$session->header();
	print $session->header(-location=>"../ConfermaInserimentoSala.cgi");
}
