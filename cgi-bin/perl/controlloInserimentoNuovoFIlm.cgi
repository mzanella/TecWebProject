...
@return = FilmFormControl::controlloPaese(\%input);
$errors{'errPaese'}=$return[0];
$error=$error+$return[1];

@return = FilmFormControl::controlloDurata(\%input);
$errors{'errDurata'}=$return[0];
$error=$error+$return[1];

@return = FilmFormControl::controlloRegia(\%input);
$errors{'errRegia'}=$return[0];
$error=$error+$return[1];

@return = FilmFormControl::controlloAttori(\%input);
$errors{'errAttori'}=$return[0];
$error=$error+$return[1];

@return = FilmFormControl::controlloTrama(\%input);
$errors{'errTrama'}=$return[0];
$error=$error+$return[1];

@return = FilmFormControl::controlloData(\%input);
...
if ($error>0){
	...
	$session->header();
	print $session->header(-location=>"../InserisciFilmUscita.cgi");
} elsif ($error==0){
	...
	DBFunctions::aggiungiFilmProssimamente($src, $input{'locDescr'}, $input{'titolo'}, $dataUscita,
		$input{'paeseDiProduzione'}, $input{'durata'}, $input{'trama'}, $input{'genere'}, $input{'annoProduzione'},
		"no", $input{'attori'}, $input{'regia'});
	...
	my $cookie1 = CGI::Cookie->new(-name=>'CGISESSID',-value=>$session->id);
	$session->header();
	print $session->header(-location=>"../ConfermaInserimento.cgi");
}
